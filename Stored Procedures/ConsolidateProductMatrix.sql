...
CREATE PROCEDURE [Retail].[ConsolidateProductMatrix] 
    @HouseholdType nvarchar(50),
	@RegularRandom nvarchar(10),
	@iSetPercentile int = 5,
	@ProdGenList ToBeGeneratedProdList READONLY,
...
AS
	...
	DECLARE @TempCompBuyMatrix TABLE
			(TimeSliceId int,
			TimeSlice nvarchar(50),
			TopCategoryId int,
			TopCategory nvarchar(200),
	...
	DECLARE
			...
			@AgeGroupType nvarchar(50),
			@Gender nvarchar(10),
			@iCount int = 1, @iTotalRow int = 0;



	Select @iTotalRow = count(*) from @ProdGenList
	WHILE @iCount <= @iTotalRow
	BEGIN
		Select @Profile =Profile, @AgeGroupType = AgeGroupType, @Gender = Gender
			from @ProdGenList where id = @iCount


			----------original product matrix logic ---------------------------------------------------------
			SELECT @iBuyCategory = 1, @totalBuyMatrix = 0;

			IF LEN(LTRIM(RTRIM(@Profile))) <= 0
				 SET @Profile = @AgeGroupType + @Gender

			IF OBJECT_ID('tempdb..#tmpBuyMatrix') IS NOT NULL DROP TABLE #tmpBuyMatrix
		...
		Insert into @ComputedBuyMatrix select @Profile, @AgeGroupType, @Gender, * from @TempCompBuyMatrix

		DELETE from @TempCompBuyMatrix
